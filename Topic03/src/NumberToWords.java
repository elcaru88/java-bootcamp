...
public class NumberToWords {
	...
	public NumberToWords(String userNumber) {
		this.n = userNumber;
			...
			public String ConvertAllToWords() {  
				String words;
				Float num = new Float(n) ;
				int dollars = (int)Math.floor( num ) ;
				int cent = Integer.parseInt(n.substring(n.lastIndexOf(".")+1));
				if (dollars != cent) {
					words=convertNumberToWords(dollars)+ " and "+ cent + "/100 dollars";
				}else {
					words=convertNumberToWords(dollars)+ " dollars";
				}
			...
			public static void main(String[] args) {
				...
				NumberToWords nw = new NumberToWords(userNumber);
				System.out.println(nw.ConvertAllToWords());
				in.close();
			}
